...
(function init3DBackground() {
    ...
    let accentColorStr = rootStyle.getPropertyValue('--accent').trim();
    if (!accentColorStr) accentColorStr = '#e0e4eb'; // Fallback
    ...
    // We update the lines dynamically in the animation loop
    ...
    // Listen for theme switch to update color dynamically
    const themeToggle = document.getElementById('themeToggle');
    if (themeToggle) {
        themeToggle.addEventListener('change', () => {
            setTimeout(() => {
                const newAccentStr = getComputedStyle(document.documentElement).getPropertyValue('--accent').trim();
                const newColor = new THREE.Color(newAccentStr || '#e0e4eb');
    ...
    function animate() {
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            if (positions[i * 3 + 2] > 0 || positions[i * 3 + 2] < -15) velocities[i].z *= -1;
        }
        
        particles.geometry.attributes.position.needsUpdate = true;

        // Draw connections natively to save overhead by using a dynamic LineSegments
        const linePositions = [];
        ...
        // Recreate lines geometry each frame
        ...
        // Mouse interaction (smooth rotational tilt)
